...
GitHub Actionsにより、5分ごとに `/api/instagram/sync` を呼び出し、サーバー側でその時間帯の未実行分だけ同期します。実際の定時同期は毎時17分基準で判定し、GitHub Actionsの一時的な遅延や取りこぼしがあっても次の実行で回収しやすくしています。

VercelのHobbyプランはCronを1日1回までしか実行できないため、1時間ごとの同期には `.github/workflows/instagram-hourly-sync.yml` を使用します。
...
4. GitHubの `Actions` → `Instagram hourly sync` → `Run workflow` で初回を手動実行し、成功を確認します。
