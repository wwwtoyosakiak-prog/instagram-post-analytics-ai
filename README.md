...
同期APIは `GET /api/instagram/full-sync` または `POST /api/instagram/full-sync` です。Instagram Business Accountの `/media` を取得し、投稿ごとの `/insights` を取得します。
...
GitHub Actionsにより、`/api/instagram/full-sync` を1日4回呼び出します。

実行時刻は毎日 `00:17`、`06:17`、`12:17`、`18:17`（日本時間）です。

VercelのHobbyプランはCronを1日1回までしか実行できないため、定期同期はVercel Cronを使わず、`.github/workflows/instagram-hourly-sync.yml` だけで実行します。

`vercel.json` にはInstagram定期同期のCronを入れず、Vercel側には1日1回のトークン更新Cronだけを残します。

1. VercelのEnvironment Variablesに、推測されにくい長い文字列を設定します。

```env
CRON_SECRET=replace-with-a-long-random-string
```

2. GitHubリポジトリの `Settings` → `Secrets and variables` → `Actions` → `New repository secret` を開きます。
3. Nameを `CRON_SECRET`、SecretをVercelに設定したものと同じ文字列にして保存します。
4. GitHubの `Actions` → `Instagram scheduled sync` → `Run workflow` で初回を手動実行し、成功を確認します。

同期結果はSupabaseの `instagram_posts` に更新保存され、`instagram_post_insight_snapshots` には実行時点の数値が毎回追加されます。
...
Instagram自動同期の実行履歴、最終同期時刻、同期エラー表示を有効にする場合は、以下のファイルをSQL Editorで実行します。

```text
supabase/add-instagram-sync-runs.sql
```
