...
create table if not exists public.app_users (
  ...
  password_hash text not null, role text not null default 'member' check (role in ('admin', 'member')),
  active boolean not null default true, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
create table if not exists public.instagram_user_connections (
  owner_id text primary key, access_token text not null, instagram_user_id text, username text,
...
create table if not exists public.app_data_backups (
  id uuid primary key default gen_random_uuid(), owner_id text not null, backup_date date not null,
