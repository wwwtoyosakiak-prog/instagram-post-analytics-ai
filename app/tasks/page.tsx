...
import { postCategoryLabels, taskStatusLabels, taskStatusOptions } from "@/lib/metrics";
          ...
          <div className="grid gap-3">
            ...
            <div>
              ...
              <select value={form.postId ?? ""} onChange={(event) => setForm({ ...form, postId: event.target.value || undefined })}>
                ...
                {posts.map((post) => (
                  <option key={post.id} value={post.id}>{post.date} / {postCategoryLabels[post.category ?? "other"]}</option>
                ))}
              </select>
            </div>
            <div>
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2 text-xs text-stone-600">
                        ...
                        {post ? <Link href={`/posts/detail?id=${post.id}`} className="font-semibold text-clay hover:underline">投稿: {post.date} / {postCategoryLabels[post.category ?? "other"]}</Link> : <span>投稿: 紐づけなし</span>}
                      </div>
