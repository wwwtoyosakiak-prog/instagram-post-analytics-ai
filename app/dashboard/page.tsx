...
type MetricSource = 'api' | 'manual';

// ── 統合ロジック ──────────────────────────────────────────

/**
 * APIの値が存在かつ>0ならAPI採用、それ以外は手入力にフォールバック。
 * preferApi=false で手入力優先に切り替え可能。
 */
function mergePostMetrics(
  post: InstagramPost,
...
function matchPostToMedia(post: InstagramPost, mediaList: ApiMedia[]): ApiMedia | undefined {
  // 1. permalink 完全一致
  if (post.url) {
