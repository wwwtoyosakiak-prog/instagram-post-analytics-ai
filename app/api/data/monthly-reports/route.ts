...
export async function GET(request: NextRequest) {
  ...
  const reports = await listMonthlyReportsFromSupabase(accountId, month, getAuthenticatedUser(request));
...
export async function POST(request: NextRequest) {
  ...
  const report = await createMonthlyReportInSupabase(body.report as MonthlyReport, body.accountId ?? null, body.accountName ?? "すべて", getAuthenticatedUser(request));
